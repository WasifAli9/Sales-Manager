...
router.post("/sales-targets/email", async (req, res): Promise<void> => {
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#0B1220;color:#e2e8f0;border-radius:16px">
        ...
        <p style="color:#475569;font-size:12px;margin:0">Generated by Closer · ${new Date().toLocaleDateString()}</p>
      </div>
    `,
    attachments: [
      {
  ...
  logger.info({ productId, year, recipientEmail }, "Sales targets email sent");
